...
const MemoizedReactMarkdown: FC<{ content: string; role?: string }> = memo(
  ({ content, role }) => {
    ...
    return (
      <div className={`w-full flex ${isUser ? 'justify-end' : 'justify-start'}`}>
        <ReactMarkdown
          className={cn(`prose dark:prose-invert prose-p:leading-loose prose-pre:p-0 break-words custom-markdown bg-neutral-100/90  dark:!bg-neutral-700 p-4 rounded-2xl `,
            isUser ? 'max-w-[60%] float-right' : 'max-w-[90%]',
          ...
          components={{
            p: ({ children }) => (
              <p className="mb-2 last:mb-0 block leading-relaxed whitespace-pre-wrap">{children}</p>
            ),
  ...
  (prevProps, nextProps) => prevProps.content === nextProps.content && prevProps.role === nextProps.role
);
