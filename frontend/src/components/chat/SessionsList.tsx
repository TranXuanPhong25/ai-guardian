...
const ChatSessionsList: React.FC<ChatSessionsListProps> = ({
  ...
  return (
    <div className="flex flex-col overflow-y-auto h-full mb-4 space-y-2">
      <h1 className='text-lg font-semibold text-gray-700 dark:text-gray-100 px-2'>
        Chats
      </h1>
      {chatSessions.map((session) => (
        <Card
          key={session.id}
          className={cn(
            'hover:bg-neutral-200/50 hover:dark:bg-neutral-700 cursor-pointer border-none shadow-none group rounded-full px-2 bg-neutral-100',
            session.id === selectedSessionId &&
            'bg-neutral-200/80 dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700'
          )}
        ...
        >
          <CardContent className={cn('p-2 flex items-center justify-between')}>
            {editingSessionId === session.id ? (
              <input
                ref={renameInputRef}
                className="flex-1 w-[90%] whitespace-nowrap text-clip p-1 border-none focus:outline-neutral-200 focus:dark:outline-neutral-600"
                value={newTitle}
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent
                align="end"
                className="dark:bg-neutral-900 group-hover:flex"
                onCloseAutoFocus={(e) => e.preventDefault()}
              >
                <DropdownMenuItem
                  className="cursor-pointer hover:bg-neutral-300 hover:dark:bg-neutral-100 p-2 rounded-lg"
                  onClick={(e) => {
                    e.stopPropagation();
                ...
                <DropdownMenuItem
                  className="cursor-pointer hover:bg-neutral-300 hover:dark:bg-neutral-100 p-2 rounded-lg text-red-500 hover:!text-red-600"
                  onClick={(e) => {
                    e.stopPropagation();
